...
class BotArchive(object):
    def archive(self, media_id, undo=False):
        ...
        if super(self.__class__, self).archiveMedia(media, undo):
            ...
            return True
        self.logger.info("Media with %s is not %s ." %
                         media_id, 'unarchived' if undo else 'archived')
        return False
    ...
    def archive_medias(self, medias):
        ...
        for media in tqdm(medias):
            if not self.archive(media):
                ...
                break
        self.logger.info("DONE: Total archived %d medias." %
                         self.total_archived)
        return broken_items
    ...
    def unarchive_medias(self, medias):
        ...
        for media in tqdm(medias):
            if not self.unarchive(media):
                ...
                broken_items = medias[medias.index(media):]
                break
        self.logger.info("DONE: Total unarchived %d medias." %
