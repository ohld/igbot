...
def getAmountDistribution(self):
    ...
    initialActionAmount = getInitialActionAmount(self)

    while foundRightCategory!=True and iteration<securityBreak and len(categories)>0:
        ...
        self.logger.info("getAmountDistribution: Selected category: %s, iteration %s, daysForThisCategory: %s" % (
        categories[selectedCategoryIndex], iteration, daysForThisCategory))
        ...
        query = " select count(*) as total from  (select count(*) as total, date(timestamp) from bot_action " \
                ...
                " group by date(timestamp) having (count(*)>%s) " \
                " order by date(timestamp) desc) my_table"

        #todo calculate the value of 1000 based on category type
        result = api_db.fetchOne(query, self.id_campaign, 1000)
        self.logger.info(result['total'])
