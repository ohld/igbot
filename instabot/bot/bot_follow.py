...
class BotFollow(object):
    def follow(self, user_id):
        user_id = self.convert_to_user_id(user_id)
        print('\n ===> Going to Follow user_id: %s ' %
    ...
    def follow_users(self, user_ids):
        ...
        user_ids = list(
            (set(user_ids) - set(followed_list)) - set(skipped_list))
    ...
    def follow_followers(self, user_id, nfollows=None):
        ...
        if not follower_ids:
            self.logger.info(
                "%s not found / closed / has no followers." % user_id)
        else:
    ...
    def follow_following(self, user_id, nfollows=None):
        ...
        if not following_ids:
            self.logger.info(
                "%s not found / closed / has no following." % user_id)
        else:
