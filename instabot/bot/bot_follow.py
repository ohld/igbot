...
def follow(self, user_id):
    ...
    if limits.check_if_bot_can_follow(self):
        ...
        if super(self.__class__, self).follow(user_id):
            ...
            print('\n\033[92m Writing user_id to file : followed.txt ... \033[0m')      #Log to console
...
def follow_users(self, user_ids):
    ...
    followed_list = self.read_list_from_file("followed.txt")                   # Read followed.txt file 
    skipped_list = self.read_list_from_file("skipped.txt")                     # Read skipped.txt file 
    print('\n\033[92m Going to follow %s user_ids ...\033[0m' % len(user_ids)) # Log to console
    user_ids = list((set(user_ids) - set(followed_list)) - set(skipped_list))  # remove skipped and followed list from user_ids
    print('\n\033[92m After filtering followedlist.txt and skippedlist.txt, [ %s ] user_ids left to follow. \033[0m' % len(user_ids)) # Log to console
