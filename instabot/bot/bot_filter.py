...
class BotFilter(object):
    ...
    def filter_medias(self, media_items, filtration=True, quiet=False, is_comment=False):
        if filtration:
            ...
            if not is_comment:
                ...
                media_items = self._filter_medias_not_commented(media_items)
            if not quiet:
                self.logger.info(
                    "After filtration %d medias left." % len(media_items))
    ...
    def check_media(self, media_id):
        ...
        if len(self.filter_medias(self.LastJson["items"])):
            return self.check_user(self, self.get_media_owner(media_id))
        else:
            return False
    ...
    def check_user(self, user_id, filter_closed_acc=False):
        ...
        if "is_business" in user_info:
            if user_info["is_business"]:
                ...
                self.skippedlist_adder(user_id)  # Add user_id to skipped.txt
                return False
        ...
        if "follower_count" in user_info and "following_count" in user_info:
            ...
            try:
                if user_info["follower_count"] / user_info["following_count"] \
                    ...
                    # Add user_id to skipped.txt
                    self.skippedlist_adder(user_id)
                    return False
        ...
        if self.search_stop_words_in_user(user_info):
            # Log to Console
