...
if args.locations:
    for location in args.locations:
        ...
        finded_location = bot.last_json['items'][0]
        if finded_location:
    ...
    bot.search_location(location_name)
    if not bot.last_json['items']:
        print(u'Location was not found')
        exit(1)
    ncomments = args.amount or input(u"How much comments per location?\n")
    ans = True
    while ans:
        for n, location in enumerate(bot.last_json["items"], start=1):
        ...
        try:
            ans = int(ans) - 1
            if ans in range(len(bot.last_json["items"])):
                comment_location_feed(bot, bot.last_json["items"][ans], amount=int(ncomments))
        except ValueError:
