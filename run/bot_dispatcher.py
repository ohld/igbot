...
try:
    ...
    #to do exlude bot
    usersLikeForLike = api_db.fetchOne('select count(*) as total_users  from users join user_subscription on (users.id_user = user_subscription.id_user)  join campaign on (users.id_user = campaign.id_user) join user_rol on (users.id_user=user_rol.id_user) where (user_subscription.end_date>now() or user_subscription.end_date is null)   and campaign.id_campaign!=%s and user_rol.rol_id=1 order by users.id_user',args.angie_campaign)
